...
class MqttPublisherDSPlugin(EventPlugin, PluginConfigMixin):
    PLUGIN_ID = "MQTT Status (DS)"
    PLUGIN_NAME = _("MQTT Publisher (DS)")
    ...
    def plugin_on_song_started(self, song):
        self.song = song
        cover = (app.cover_manager.get_cover(song) if song
            else '')
        ...
        cover_alias = self.config_get(*Config.COVER_ALIAS)
        status = status.replace(cover_alias, (cover.name if cover else ''))
        self._set_status(status)

    def plugin_on_paused(self):
        cover = (app.cover_manager.get_cover(self.song) if self.song
                 else '')
        pat_str = self.config_get(*Config.PAT_PAUSED)
        pattern = Pattern(pat_str)
        status = (pattern.format(self.song) if self.song
    ...
    _CONFIG = [
        ...
        (_("Cover file substitution pattern"),
         Config.COVER_ALIAS,
         _("Plain text pattern will be substituted by cover file name"))
    ]
